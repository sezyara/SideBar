...
import ru.sezex.sidebar.adapter.AppChooserDialog;
...
public class SidebarMenuOptions {
	...
	public static void showGroupCreatorDialog(final AppListActivity activity,
			final Context c, boolean use_values) {
		...
		final TextView tV_warning = (TextView) dialog
		...
		final View.OnClickListener listener = new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				case R.id.btn_create:
					...
					editor.putInt(mPkg0 + Common.SEPARATOR_GROUP + mPkg1, pref
							.getAll().size());
					editor.commit();
					Util.toast(
							c,
							c.getResources().getString(
									R.string.create_group_done)
									+ "\n\n" + mPkg0 + "\n" + mPkg1);
					if (activity != null) {
						activity.updateList();
					} else {
