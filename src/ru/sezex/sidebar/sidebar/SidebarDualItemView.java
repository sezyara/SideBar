...
public abstract class SidebarDualItemView extends SidebarItemView {
	...
	public SidebarDualItemView(final SidebarService service,
		...
		if (TextUtils.isEmpty(mPkg1) || TextUtils.isEmpty(mPkg2)) {
			...
			try {
				...
				final Intent intent1 = new Intent(
						pm.getLaunchIntentForPackage(mPkg1));
				final Intent intent2 = new Intent(
						pm.getLaunchIntentForPackage(mPkg2));

				final int width = getContext().getResources()
						.getDisplayMetrics().widthPixels;
				final int height = getContext().getResources()
						.getDisplayMetrics().heightPixels;
				final boolean landscape = width > height;

				IntentUtil.launchIntent(getContext(), intent1,
						landscape ? IntentUtil.SIDE_LEFT : IntentUtil.SIDE_TOP);
				IntentUtil.launchIntent(getContext(), intent2,
						landscape ? IntentUtil.SIDE_RIGHT
								: IntentUtil.SIDE_BOTTOM);
