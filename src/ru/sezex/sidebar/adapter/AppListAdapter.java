...
public abstract class AppListAdapter extends BaseAdapter {
						...
						if (pkg_name.contains(Common.SEPARATOR_GROUP)) {
							try {
								...
								final Drawable icon1 = ai1.loadIcon(
										mPackageManager).mutate();
								item.icon = Util.layerTwoIcons(mContext, icon0,
										icon1);
							...
							try {
								ApplicationInfo ai = mPackageManager
										.getApplicationInfo(pkg_name, 0);
								final PackageItem item = new PackageItem();
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView != null) {
			...
			convertView = mLayoutInflater.inflate(R.layout.view_package_list,
					parent, false);
