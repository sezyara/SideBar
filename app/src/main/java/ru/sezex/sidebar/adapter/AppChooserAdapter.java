...
public abstract class AppChooserAdapter extends BaseAdapter implements
		Filterable {
	...
	public AppChooserAdapter(Context context) {
		...
		mInstalledAppInfo = mPackageManager
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				for (PackageInfo info : mTemporarylist) {
					...
					item.title = info.applicationInfo
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView != null) {
			...
			convertView = mLayoutInflater.inflate(R.layout.view_app_list,
					parent, false);
			...
			holder.pkg = (TextView) convertView
		...
		return new Filter() {
			...
			protected FilterResults performFiltering(CharSequence constraint) {
				...
				for (PackageInfo data : mInstalledAppInfo) {
					final String filterText = constraint.toString()
							.toLowerCase(Locale.ENGLISH);
					try {
						if (data.applicationInfo.loadLabel(mPackageManager)
